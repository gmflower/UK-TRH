...
tmeandist <- t(sapply(stage1list, function(x) x$tmeanper))
...
ladvarpred <- ladvar[seq(agevarlab),]
ladvarpred[,grep("PC", names(ladvarpred), fixed=T)] <- 0
predparage <- predict(metaall, newdata=ladvarpred, vcov=T)
...
# COLD
par(mar=c(1,5,1,1))
plot(stdrate_cold~xpoint, eff, type="n", axes=F, xlab="", ylim=c(90,160),
