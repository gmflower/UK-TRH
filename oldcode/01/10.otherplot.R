...
pclab <- c("High-income rural", "Affluent warm",
  "Poor infrastracture and pressured")

# 5TH-95TH PERCENTILES OF PCA META-VARIABLES
datapred <- apply(ladvar, 2, quantile, c(5,95)/100, na.rm=T)
datapredmean <- colMeans(ladvar)

# MULTI-PANEL PLOT
pdf("figures/expresp.pdf", height=7, width=9)
...
plot(cpall, type="n", ci="n", main="LAD-specific and pooled",
  ylim=c(0.8,2.5), ylab="RR", xlab="Temperature percentiles", xaxt="n")
...
# EFFECT OF META-VARIABLES
for(i in seq(names(ladvar))) {
  
  # DEFINE THE DATA USED FOR PREDICTION
  datanew <- as.data.frame(datapred)
  datanew[,names(ladvar)[-i]] <- rep(datapredmean[names(ladvar)[-i]], each=2)

  # PREDICT THE EXPOSURE-RESPONSE
  pred <- predict(metaall, datanew, vcov=T)
  cplow <- crosspred(bvar, coef=pred[[1]]$fit, vcov=pred[[1]]$vcov,
    model.link="log", at=tmeanper, cen=cen)
  cphigh <- crosspred(bvar, coef=pred[[2]]$fit, vcov=pred[[2]]$vcov,
    model.link="log", at=tmeanper, cen=cen)
  
  # PLOT
  plot(cplow, "overall", main=pclab[i], ylim=c(0.8,2.5), ylab="RR", 
    xlab="Temperature percentiles", xaxt="n", col=3,
    ci.arg=list(col=alpha("green", 0.2)))
  lines(cphigh, "overall", col="orchid", ci="area",
    ci.arg=list(col=alpha("orchid", 0.2)))
  labperc <- paste(c(0,1,25,75,99,100),sep="")
  axis(1, at=tmeanper[paste(labperc, ".0%", sep="")], 
    labels=paste(labperc, "%", sep=""))
  legend("top",c("Low","High"), col=c("green","orchid"), lty=1, ncol=2, bty="n")
}
layout(1)
dev.off()
...
layout(t(1:2))
corrplot(corvarlad, tl.cex=0.8, mar=c(0,1,2,1), title="LAD level")
corrplot(corvarlsoa, tl.cex=0.8, mar=c(0,1,2,1), title="LSOA level")
dev.off()
...
# PLOTS COORDINATES
pdf("figures/pcascore.pdf", height=5, width=6)
