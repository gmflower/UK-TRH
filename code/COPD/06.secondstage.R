...
###############################################################################
# COPD study replication exercise:


coef_all_ages <-  unlist(lapply(seq(stage1list), function(i) 
    t(sapply(stage1list[[i]]$clist[["copd"]], "[[", "coefall"))))
vcov_all_ages <-  unlist(lapply(seq(stage1list), function(i) 
    t(sapply(stage1list[[i]]$clist[["copd"]], "[[", "vcovall"))))
  
#coef_age6574 <- unlist(lapply(seq(stage1list), function(i) stage1list[[i]]$clist[["copd"]][["age6574"]][["coefall"]][["cbtemp"]]))
#vcov_age6574 <- unlist(lapply(seq(stage1list), function(i) stage1list[[i]]$clist[["copd"]][["age6574"]][["vcovall"]]))


library(metafor) 
rma(yi=coef_all_ages, vi=vcov_all_ages, measure="RR")
#rma(yi=coef_age6574, vi=vcov_age6574, measure="RR")
