...
fcastreslist <- foreach(i=seq(listlsoa), .packages=pack) %dopar% {
  ...
  # COMPUTE TEMPERATURE PERCENTILES (NB: USE THE LAD LEVEL - APPROXIMATION)
  lsoaper <- stage1list[[lookup$LAD11CD[i]]]$lsoaper
  tmeanper <- as.numeric(lsoaper[lsoaper$LSOA11CD == listlsoa[i], -1])
...
lsoatmax <- do.call(rbind, lapply(stage1list, function(x) 
  data.table(x$lsoaper[,c("LSOA11CD","100%")])))
